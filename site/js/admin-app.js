...
function renderFixturesSection() {
  ...
  renderFixtures(state, elements.fixtures, {
    ...
    onFirstScorerResultChange: (matchId, value) => {
      const match = state.matches.find(m => m.id === matchId);
      if (match) match.firstScorerResult = value;
      renderFixturesSection();
      renderPayoutTables(state, elements);
    },
    onFirstScorerPredictionChange: (matchId, name, value) => {
      const match = state.matches.find(m => m.id === matchId);
      if (match) {
        if (!match.firstScorerPredictions) match.firstScorerPredictions = {};
        match.firstScorerPredictions[name] = value;
      }
      renderPayoutTables(state, elements);
    }
  });
...
function addParticipant() {
  ...
  state.matches.forEach(m => {
    ...
    m.predictions[name] = "";
    if (!m.firstScorerPredictions) m.firstScorerPredictions = {};
    m.firstScorerPredictions[name] = "";
  });
...
function removeParticipant(name) {
  ...
  state.matches.forEach(m => {
    ...
    if (m.firstScorerPredictions) delete m.firstScorerPredictions[name];
  });
  render();
}
...
function addMatch() {
  ...
  state.matches.push({
    ...
    firstScorerResult: "",
    firstScorerPredictions: Object.fromEntries(state.participants.map(p => [p, ""]))
  });
